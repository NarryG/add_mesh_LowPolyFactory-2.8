...
class LPF_UpdateLightingButton(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        for o in bpy.data.objects:
            if o.type == 'LIGHT':
                c.collection.objects.unlink(o)
                c.blend_data.objects.remove(o)

        # Create new lamp
        lamp_data = bpy.data.lights.new(name="Light", type='SUN')
        lamp_object = bpy.data.objects.new(
            name="Light",
            object_data=lamp_data)
        c.collection.objects.link(lamp_object)
        ...
        c.view_layer.objects.active = lamp_object
